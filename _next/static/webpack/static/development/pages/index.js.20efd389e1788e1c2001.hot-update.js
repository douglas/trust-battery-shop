...
function calculateScore(reliability, credibility, intimacy, self_orientation) {
  return (reliability + credibility + intimacy) / calculateD(self_orientation);
}

function calculateD(score) {
  var aarons_chart = [48, 51.2, 54.9, 59.1, 64, 69.8, 76.8, 85.3, 96, 109.7, 128, 153, 192, 256, 384, 769, 10000000];
  var index = 16 - score;
  return aarons_chart[index];
}
...
function calculatePercentage(score) {
  ...
  var max_score_sum = max_reliability + max_credibility + max_intimacy + max_self_orientation;
  var max_score = max_reliability + max_credibility + max_intimacy;
...
var totalPointsFromMilestoneMap = function totalPointsFromMilestoneMap(milestoneMap) {
  ...
  return (calculateScore(reliability, credibility, intimacy, self_orientation) * 100).toFixed(0); // return trackIds.map(trackId => milestones[milestoneMap[trackId]].points).reduce((sum, addend) => (sum + addend), 0)
};
...
//# sourceMappingURL=index.js.20efd389e1788e1c2001.hot-update.js.map